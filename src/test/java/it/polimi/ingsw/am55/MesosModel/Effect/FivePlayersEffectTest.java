...
/**
 * Unit tests for  FivePlayersEffect.
 * Verifies the correct distribution of food bonuses based on the player's position
 * in the turn order and the presence of specific building cards during a 5-player game.
 */
class FivePlayersEffectTest {
    /**
     * Tests food assignment for a player in the first position (index 0) who owns a valid building.
     * Expected result: The player receives 4 food tokens (base bonus + building bonus).
     */
    @Test
    void testAddFoodWithBuldingOnFirstPosition(){
        ...
        FivePlayersEffect fivePlayersEffect = new FivePlayersEffect();
        assertEquals(0,player.getNumFoods());
        fivePlayersEffect.applyFood(player,0);
     ...
     * Expected result: The player receives 3 food tokens (base bonus only).
     */
    @Test
     ...
     * Expected result: The player receives 2 food tokens (base bonus + building bonus).
     */
    @Test
    void testAddFoodWithBuldingOnSecondPosition(){
        ...
        FivePlayersEffect fivePlayersEffect = new FivePlayersEffect();
        assertEquals(0,player.getNumFoods());
        fivePlayersEffect.applyFood(player,1);
     ...
     * Expected result: The player receives 1 food token (base bonus only).
     */
    @Test
